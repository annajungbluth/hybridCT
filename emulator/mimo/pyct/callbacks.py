...
class OutputMonitor(pl.Callback):
    ...
    def _log_matrix(
        ...
        img_data: Tensor with the shape batch x channels x M x N
        log_name: name of logged image, must contain a formatting placeholder `{channel}`
        """
        index_data = img_data[:max_images, ...]
        index_data = index_data.flatten(start_dim=1,end_dim=2)
        index_data = index_data[:, np.newaxis, ...]
        index_grid = torchvision.utils.make_grid(index_data, nrow=9)
        #img_color = colorize(index_grid, vmin=vmin, vmax=vmax, cmap=cmap)

        if isinstance(logger, pl.loggers.wandb.WandbLogger):
            images = wandb.Image(index_grid, mode='F')
            wandb.log({log_name: images}, step=global_step)
        if isinstance(logger, pl.loggers.tensorboard.TensorBoardLogger):
            logger.experiment.add_image(log_name, index_grid, dataformats="HWC", global_step=global_step)
        else:
    ...
    def on_train_batch_end(
        ...
        if (batch_idx + 1) % trainer.log_every_n_steps == 0:
            ...
            self._log_image(
                img_data=outputs["preds"], mask=outputs.get('mask', None), log_name="train/predicted", **kwargs
            )
            self._log_image(
                img_data=outputs["label"], mask=outputs.get('mask', None), log_name="train/true", **kwargs
            )
            self._log_error_map(
                err_map=outputs["err_map"], mask=outputs.get('mask', None), log_name="train/error", **kwargs
            )
            self._log_std_map(
                std_map=outputs["aleatoric_std_map"], mask=outputs.get('mask', None), log_name="train/aleatoric_std", **kwargs
            )
    ...
    def on_validation_batch_end(
        ...
        if batch_idx % trainer.log_every_n_steps == 0:
            ...
            self._log_image(
                img_data=outputs["preds"], mask=outputs.get('mask', None), log_name="val/predicted", **kwargs
            )
            self._log_image(
                img_data=outputs["label"], mask=outputs.get('mask', None), log_name="val/true", **kwargs
            )
            self._log_error_map(
                err_map=outputs["err_map"], mask=outputs.get('mask', None), log_name="val/error", **kwargs
            )
            self._log_std_map(
                std_map=outputs["aleatoric_std_map"], mask=outputs.get('mask', None), log_name="val/aleatoric_std", **kwargs
            )
            if 'epistemic_std_map' in outputs:
                self._log_std_map(
                    std_map=outputs["epistemic_std_map"], mask=outputs.get('mask', None), log_name="val/epistemic_std", **kwargs
                )
