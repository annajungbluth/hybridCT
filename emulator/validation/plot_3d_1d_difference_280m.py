...
if __name__ == "__main__":
    ...
    #plt.tight_layout()
    plt.savefig(f"{plot_dir}/rad_3D_1D_diff_{time:.1f}h_280m.png")
    plt.show()
    ...
    for i in range(3):
        ...
        ax[i].set_aspect('equal')
    #plt.tight_layout()
    plt.savefig(f"{plot_dir}/tau_3D_1D_diff_{time:.1f}h_280m.png")
    plt.show()
    ...
    ax = scatter_density(ax, tau_280m[cloud_mask][nan_mask], tau_ret_280m[cloud_mask][nan_mask], "True", "Predicted", title="1D LUT", stratified=True)#, n_sample=len(tau_ret[cloud_mask][nan_mask])) #, y_name
    plt.savefig(f"{plot_dir}/tau_pred_lut_scatter_{time:.1f}h_280m.png")
    plt.show()
    ...
    plt.savefig(f"{plot_dir}/tau_pred_unet_scatter_{time:.1f}h_280m.png")
    plt.show()
    ...
    plt.savefig(f"{plot_dir}/tau_pred_lut_scatter_<{thres}_{time:.1f}h_280m.png")
    plt.show()

    fig = plt.figure(figsize=(5,2.5), dpi=200, constrained_layout=True)
    ax = fig.add_subplot(111)
    ax = scatter_density(ax, tau_280m[cloud_mask], tau_pred_280m.data[cloud_mask], "True", "Predicted", title="U-Net", stratified=True)
    plt.savefig(f"{plot_dir}/tau_pred_unet_scatter_<{thres}_{time:.1f}h_280m.png")
    plt.show()


    tau_pred_280m_ds = xr.open_dataset(f"{data_path}/predicted_data/MISR_280m_80x80km_{time:.1f}h_optical_thickness_predicted{tag}.nc")
    vza_directions = {
            "aft": np.array([26.1, 45.6, 60.0, 70.5]),
            "fwd": -np.array([26.1, 45.6, 60.0, 70.5]),
            }
    labels = [f'({chr(97 + i)})' for i in range(8)]
    cam_labels = ["Aa", "Ba", "Ca", "Da", "Af", "Bf", "Cf", "Df"]
        ...
        for i, direction in enumerate(["aft", "fwd"]):
            for j, vza in enumerate(vza_directions[direction]):
                ...
                if tau_thres is None:
                    ...
                    cloud_mask = (tau_280m > tau_min) * (tau_pred_280m > 2) * (tau_280m <= tau_thres+0.01)
                scatter_density(ax[j,i], tau_280m[cloud_mask], tau_pred_280m[cloud_mask], "True", "Predicted", title="", stratified=True)
                ax[j,i].text(-0.2, 1.05, labels[counter], transform=ax[j,i].transAxes, fontsize='medium', fontweight='bold', va='bottom')
                ax[j,1].set_ylabel("")
                ax[j,i].set_xlabel("")
                ax[j,i].set_title(f"{cam_labels[counter]}, VZA={vza:.1f}"+r"$^{\circ}$")
                counter+=1
            ax[-1,i].set_xlabel("True")
        if tau_thres is None:
            plt.savefig(f"{plot_dir}/tau_pred_unet_scatter_offnadir_{time:.1f}h_280m.png")
        else:
            plt.savefig(f"{plot_dir}/tau_pred_unet_scatter_<{tau_thres}_offnadir_{time:.1f}h_280m.png")
        plt.show()
